...
public class Trolleybus extends Transport {
    ...
    public void run(){
        ...
        if ((getType().equals("UNKNOWN")) || (getRoute().equals("UNKNOWN")) || (getInterval() <= 0)){
            setStopFlag(true);
            System.out.println("Некорректно заполнена информация для Троллейбуса.");
        }
        else {
            setStopFlag(false);
        }
        ...
        while (!stopFlag) {
            ...
            if (new Random().nextInt(101) <= crashChance){
                ...
                System.out.println(MessageFormat.format("С Троллейбусом на маршруте \"{0}\" произошла поломка.\n" +
                                "{1}",
                        getRoute(),
                        message));
            }
