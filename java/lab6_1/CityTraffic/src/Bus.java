...
public class Bus extends Transport {
    ...
    public void run(){
        ...
        if ((getType().equals("UNKNOWN")) || (getRoute().equals("UNKNOWN")) || (getInterval() <= 0)){
            setStopFlag(true);
            System.out.println(Tools.getLocalization().getProperty("bus_incorrectData"));
        }
        else {
            setStopFlag(false);
        }
        ...
        while (!stopFlag) {
            ...
            if (new Random().nextInt(101) <= crashChance){
                ...
                System.out.println(MessageFormat.format(Tools.getLocalization().getProperty("bus_crash"),
                        getRoute(),
                        message));
            }
            else{
